import { FastifyReply, FastifyRequest } from "@src/utils/types.ts";
...
export async function createTodoHandler(request: FastifyRequest<CreateTodoSchema>, reply: FastifyReply<CreateTodoSchema>) {
...
export async function updateTodoHandler(request: FastifyRequest<UpdateTodoSchema>, reply: FastifyReply<UpdateTodoSchema>) {
...
export async function deleteTodoHandler(request: FastifyRequest<DeleteTodoSchema>, reply: FastifyReply<DeleteTodoSchema>) {
  const { params } = request

  await db
    .update(listItems)
    .set({deletedAt: sql.sql`now()`})
    .where(sql.eq(listItems.id, params.id));
